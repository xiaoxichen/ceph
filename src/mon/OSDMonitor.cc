...
int OSDMonitor::crush_ruleset_create_erasure(const string &name,
					     ...
					     stringstream &ss)
{
  int rule_id = osdmap.crush->get_rule_id(name);
  if (rule_id != -ENOENT) {
    *ruleset=osdmap.crush->get_rule_mask_ruleset(rule_id);
    return -EEXIST;
  }
  ...
  rule_id = newcrush.get_rule_id(name);
  if (rule_id != -ENOENT) {
    return -EALREADY;
  } else {
  ...
  if (*crush_ruleset < 0) {
	...
	if (ruleset_name == "") {
	  ...
	  *crush_ruleset = osdmap.crush->get_osd_pool_default_crush_replicated_ruleset(g_ceph_context);
	  ...
	  if (ret != -ENOENT) {
	    // found it, use it
	    *crush_ruleset = osdmap.crush->get_rule_mask_ruleset(ret);
	  } else {
	    CrushWrapper newcrush;
	    _get_pending_crush(newcrush);
