...
const int omap_batch = 25;
void get_omap_batch(ObjectMap::ObjectMapIterator &iter, map<string, bufferlist> &oset)
{
  oset.clear();
  for (int count = omap_batch; count && iter->valid(); --count, iter->next()) {
...
{
  map<string,bufferptr> aset;
  int r = store->getattrs(coll, ghobj, aset, false);
  if (r < 0) {
...
int do_set_attr(ObjectStore *store, coll_t coll, ghobject_t &ghobj, string key, string value)
{
  ObjectStore::Transaction tran;
  ObjectStore::Transaction *t = &tran;
  bufferptr bp(value.c_str(), value.length());
  bufferlist bl;
...
int do_set_omap(ObjectStore *store, coll_t coll, ghobject_t &ghobj, string key, string value)
{
  ObjectStore::Transaction tran;
  ObjectStore::Transaction *t = &tran;
  map<string, bufferlist> attrset;
  bufferptr valbp(value.c_str(), value.length());
  bufferlist valbl;

  valbl.push_back(valbp);
...
void usage(po::options_description &desc)
{
    ...
    cerr << "ceph-filestore-dump ... <object> (set-(attr|omap) <key> <value>" << std::endl;
    cerr << "ceph-filestore-dump ... <object> (set-omaphdr) <value>" << std::endl;
...
// The following code for export, info, log require omap or !skip-mount-omap
  if (it != ls.end()) {
      ...
      if (objcmd == "remove") {
        ...
        if (objcmd == "get-bytes") {
          ...
          if (vm.count("arg1") == 0 || arg1 == "-") {
            ...
            int fd = open(arg1.c_str(), O_RDONLY|O_LARGEFILE, 0666);
            if (fd == -1) {
              cerr << "open " << arg1 << " " << cpp_strerror(errno) << std::endl;
              ret = 1;
              goto out;
	...
	if (vm.count("arg1") == 0 || vm.count("arg2") == 0)
	  usage(desc);
	r = do_set_attr(fs, coll, ghobj, arg1, arg2);
	if (r)
	  ret = 1;
        goto out;
	...
	if (vm.count("arg1") == 0 || vm.count("arg2") == 0)
	  usage(desc);
	r = do_set_omap(fs, coll, ghobj, arg1, arg2);
	if (r)
	  ret = 1;
        goto out;
